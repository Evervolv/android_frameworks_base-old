...
import android.os.IPerformanceManager;
...
public class PerformanceManagerService extends SystemService {
    ...
    public PerformanceManagerService(Context context) {
        ...
        String[] activities = res.getStringArray(com.android.internal.R.array.config_auto_perf_activities);
    ...
    private class PerformanceSettingsObserver extends ContentObserver {
        ...
        private final Uri APP_PERFORMANCE_PROFILES_ENABLED_URI =
                Settings.Secure.getUriFor(Settings.Secure.APP_PERFORMANCE_PROFILES_ENABLED);
        ...
        public void onChange(boolean selfChange) {
            int profile = Settings.Secure.getInt(mCR, Settings.Secure.PERFORMANCE_PROFILE, PerformanceManager.PROFILE_BALANCED);
            boolean boost = Settings.Secure.getInt(mCR, Settings.Secure.APP_PERFORMANCE_PROFILES_ENABLED, 1) == 1;
    ...
    @Override
    public void onStart() {
        publishBinderService(Context.PERFORMANCE_SERVICE, mBinder);
    ...
    private void populateProfilesLocked() {
        ...
        String[] profileNames = res.getStringArray(com.android.internal.R.array.perf_profile_entries);
        int[] profileIds = res.getIntArray(com.android.internal.R.array.perf_profile_values);
        String[] profileWeights = res.getStringArray(com.android.internal.R.array.perf_profile_weights);
        String[] profileDescs = res.getStringArray(com.android.internal.R.array.perf_profile_summaries);
    ...
    public void onBootPhase(int phase) {
        if (phase == PHASE_SYSTEM_SERVICES_READY && !mSystemReady) {
            synchronized (mLock) {
                ...
                if (hasProfiles()) {
                    if (mUserProfile == PerformanceManager.PROFILE_HIGH_PERFORMANCE) {
                        Slog.w(TAG, "Reverting profile HIGH_PERFORMANCE to BALANCED");
                        setPowerProfileLocked(PerformanceManager.PROFILE_BALANCED, true);
                    } else {
    ...
    private boolean setPowerProfileLocked(int profile, boolean fromUser) {
        ...
        if (!isProfileSame && profile != PerformanceManager.PROFILE_POWER_SAVE &&
                mActiveProfile == PerformanceManager.PROFILE_POWER_SAVE) {
         ...
         */
        if (fromUser) {
            Settings.Secure.putInt(mContext.getContentResolver(), Settings.Secure.PERFORMANCE_PROFILE, profile);
            mUserProfile = profile;
        }
        ...
        mContext.enforceCallingOrSelfPermission(
                Manifest.permission.PERFORMANCE_ACCESS, null);
    ...
    private void applyAppProfileLocked(boolean fromUser) {
        ...
        } else if (fromUser && mActiveProfile == PerformanceManager.PROFILE_POWER_SAVE) {
            // leaving LPM
